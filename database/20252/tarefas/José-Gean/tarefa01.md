...
## Questão 7.E: Construa um Diagrama ER para projetar uma base de dados de um Sistema de Controle de Freqüência de Empregados de uma organização. A base de dados não deve conter redundância de dados. O modelo ER deve ser representado com um diagrama usando Mermaid.js. O modelo deve apresentar, ao menos, entidades, relacionamentos, atributos, identificadores e restrições de cardinalidade. O modelo deve ser feito no nível conceitual, sem incluir chaves estrangeiras.
...
- iii Empregados de segundo tipo devem trabalhar em *horários fixos*. A semana de trabalho do empregado deste tipo está organizada em *turnos*. Um turno tem *horário de início e horário de fim*. O empregado pode trabalhar *dois turnos no mesmo dia da seman*a. Cada dia da semana é identificado por um *código (algo como "dom", "seg", "ter", . . . )* e tem um nome (algo como "domingo", "segunda-feira", . . . ).
...
Na modelagem conceitual de ER, quando você tem uma entidade pai (EMPREGADO) que compartilha atributos comuns (codigo, nome, email, tipo) e entidades filhas que têm atributos específicos (qt_horas_mes, horas_min_dia no caso do livre, ou turnos no fixo), isso é chamado de especialização (ou generalização do ponto de vista do pai).

"atua_em" → liga EMPREGADO_HORARIO_FIXO a TURNO

"ocorre_em" → liga TURNO a DIA_SEMANA

"registra" → liga EMPREGADO a PONTO

"referencia" → liga TURNO a PONTO
...
erDiagram
   ...
   EMPREGADO_HORARIO_FIXO }o--o{ TURNO : "atua_em"
